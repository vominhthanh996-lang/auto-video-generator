...
DEFAULT_POSITIVE_SUFFIX = (
    "clear natural human faces, anatomically correct eyes nose mouth ears and jaw, complete balanced facial features, readable facial expression, "
    ...
    "no body fragments, no fused torsos, no merged shoulders or hips, no hidden extra limbs, no clipped hands when hands are part of the beat, "
    "full intact face with both eyes, one nose, one mouth, complete ears when visible, no broken facial structure, "
...
DEFAULT_NEGATIVE = (
    ...
    "missing eye, extra eye, uneven eyes, broken eyelids, missing nose, broken nose, missing mouth, bad mouth, fused lips, extra teeth, duplicate face, two faces on one head, "
    "missing ear, extra ear, deformed ear, twisted jaw, broken jaw, melted jawline, collapsed cheeks, incomplete face, disfigured face, "
    ...
    "merged pelvis, merged shoulders, body clipping through another person, torso growing out of another torso, duplicated torso, duplicated hip, duplicated shoulder, "
...
ANATOMY_GUARDRAIL = (
    "human anatomy guardrail: every visible person must have one coherent body, one head, and cleanly separated limbs; "
    "no fused or overlapping torsos, no merged shoulders or hips, no extra person fragments, no cloned presenter, "
    "no missing hands or feet when those body parts should be readable in the shot, and no characters visually growing out of each other"
)
...
DEFAULT_MIN_IMAGE_BYTES = 24 * 1024
...
def scene_prompt(scene: dict[str, Any]) -> str:
    ...
    if "lam tich" in combined_text and any(token in combined_text for token in ["journey", "survivor column", "xe lan", "stretcher", "ration", "well", "danger", "blood", "wound", "track", "smoke", "fire"]):
        cast_notes.append("Lam Tich may stay feminine and attractive here, but use practical torn survival clothing with a fitted thick-strap weathered survival top and rugged shorts; avoid two-piece bikini silhouettes and do not let the outfit overpower the dangerous beat")
    ...
    def decorate_subject(text: str) -> str:
        low = text.lower()
        if "lam tich" in low:
            return "Lam Tich, a clearly adult beautiful wasteland woman, feminine and attractive but still practical and story-first, wearing a fitted thick-strap weathered survival top with rugged shorts, not bikini bottoms"
        if "tan da" in low:
            return "Tan Da, a clearly adult tall muscular man with upright masculine strength, dark tactical wasteland coat or heavy scavenger jacket, long rugged pants, boots, and survival gear"
        if "di man" in low:
    ...
    if detail_focus:
        ...
        face_control = (
            ...
            "separate bodies, clear spacing between characters, no fused limbs, no overlapping torsos, no tangled poses, no hidden extra body fragments"
        )
    elif named_people >= 2:
        face_control = (
            ...
            "no fixed left-right blocking unless the narration implies it, no repeated shelter tableau, no body overlap, no one merged into the other, no cloned or partial extra person"
        )
    elif reaction_focus:
        face_control = (
    ...
    if "lam tich" in combined_text and any(token in combined_text for token in ["rope", "well", "gieng", "danger", "signal pole", "claws", "ration", "bargain", "test", "smoke"]):
        prompt_parts.append("Lam Tich only can stay attractive and somewhat revealing, but the outfit must be a fitted thick-strap weathered survival top plus rugged shorts, never bikini bottoms or a two-piece swimsuit, and her pose must stay practical and survival-driven over the story beat")
    if male_only_notes:
...
def read_png_dimensions(path: Path) -> tuple[int, int]:
    ...
    if len(header) < 24 or not header.startswith(PNG_SIGNATURE):
        raise SystemExit(f"Generated file is not a valid PNG: {path}")
    width, height = struct.unpack(">II", header[16:24])
    return width, height


def validate_generated_output(output: Path, expected_width: int, expected_height: int, min_image_bytes: int) -> list[str]:
    reasons: list[str] = []
    if not output.exists():
        return ["output image file was not created"]
    file_size = output.stat().st_size
    if file_size <= 0:
        reasons.append("output image file is empty")
        return reasons
    if file_size < min_image_bytes:
        reasons.append(f"output image file is suspiciously small ({file_size} bytes)")
    suffix = output.suffix.lower()
    if suffix == ".png":
        try:
            width, height = read_png_dimensions(output)
        except SystemExit as exc:
            reasons.append(str(exc))
            return reasons
        if width != expected_width or height != expected_height:
            reasons.append(
                f"output image size mismatch: got {width}x{height}, expected {expected_width}x{expected_height}"
            )
    return reasons
...
def generate_scene(args: argparse.Namespace, scene: dict[str, Any], index: int, storyboard_dir: Path, assets_dir: Path) -> Path:
    ...
    for attempt in range(1, args.max_attempts + 1):
        seed = (args.seed + index + (attempt - 1) * 1009) if args.seed >= 0 else int(time.time() * 1000) % 2_147_483_647
        last_seed = seed
        if output.exists():
            try:
                output.unlink()
        ...
        try:
            ...
            try:
                ...
                if not copy_latest_output_image(args.comfy_output_dir, args.prefix, started_at, output):
                    raise
            validation_errors = validate_generated_output(output, expected_width, expected_height, args.min_image_bytes)
            if not validation_errors:
                break
            attempt_errors.append(f"attempt {attempt}: " + "; ".join(validation_errors))
            print(
                f"[scene {index + 1}] validator rejected image on attempt {attempt}: {'; '.join(validation_errors)}",
                flush=True,
            )
        except Exception as exc:
            attempt_errors.append(f"attempt {attempt}: {exc}")
            print(f"[scene {index + 1}] generation failed on attempt {attempt}: {exc}", flush=True)
        if output.exists():
            try:
                output.unlink()
            except OSError:
        ...
        joined = " | ".join(attempt_errors) if attempt_errors else "unknown validation failure"
        print(
            f"[scene {index + 1}] skipped after {args.max_attempts} failed image attempts: {joined}",
            flush=True,
        )
        scene.setdefault("image_generation_warnings", [])
        scene["image_generation_warnings"].append(
            {
                "scene_index": index + 1,
                "status": "skipped_after_failed_attempts",
    ...
    scene["local_image"] = {
        ...
        "validator": {
            "expected_size": f"{expected_width}x{expected_height}",
            "min_image_bytes": args.min_image_bytes,
            "attempts_used": min(args.max_attempts, len(attempt_errors) + 1),
        },
    }
    return output
...
def main() -> None:
    ...
    parser.add_argument("--max-attempts", type=int, default=DEFAULT_MAX_ATTEMPTS, help="Maximum generation attempts per scene before skipping it.")
    parser.add_argument("--retry-delay", type=float, default=1.5, help="Seconds to wait before retrying a failed scene.")
    parser.add_argument("--min-image-bytes", type=int, default=DEFAULT_MIN_IMAGE_BYTES, help="Reject outputs smaller than this many bytes.")
